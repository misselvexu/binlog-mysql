 ...
 */
public class UpdateRowsEventData extends DomainSerializable implements EventData {

    private static final long serialVersionUID = 8130853565698920758L;
    ...
    public String toString () {
        ...
        for (Map.Entry<Serializable[], Serializable[]> row : rows) {
            sb.append("\n    ").
                    append("{before=").append(Arrays.toString(parseSerializable(row.getKey()).toArray())).
                    append(", after=").append(Arrays.toString(parseSerializable(row.getValue()).toArray())).
                    append("},");
        }
        if (!rows.isEmpty()) {
